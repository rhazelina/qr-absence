...
class LoginAwal : BaseNetworkActivity() {
            ...
            object : GestureDetector.SimpleOnGestureListener() {
                ...
                override fun onFling(
                    ...
                    if (abs(diffY) > SWIPE_THRESHOLD && abs(velocityY) > SWIPE_VELOCITY_THRESHOLD) {
                        if (diffY < 0) { navigateToNext(); return true }
                    }
                    return false
                }
            }
    ...
    private fun loadSavedAppearance() {
        try {
            ...
            if (savedLogo.startsWith("content://") || savedLogo.startsWith("file://")) {
                ...
                ivLogo.setImageResource(if (resId != 0) resId else R.drawable.logo_1)
            }

            findViewById<TextView>(R.id.textView).text  = appPreferences.getJudulUtamaSync()
            findViewById<TextView>(R.id.textView2).text = appPreferences.getJudulSubSync()
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
    ...
    private fun setupSimpleDropdown() {
        ...
        val roles = arrayOf("Admin", "Waka", "Guru", "Wali Kelas", "Siswa", "Pengurus")
        var currentIndex = -1
        ...
        roleEditText.setOnClickListener { btnDropdown.performClick() }
