...
class ClassRepository(
    ...
    suspend fun getClasses(
        ...
        return ApiUtils.handleApiCall {
            apiService.getClasses(search, grade, major, available, page, perPage)
    ...
    // FIX: ApiService.getMyClassAttendance returns Response<TeacherAttendanceHistoryResponse>
    // which has NO .data field — use handleApiCallFlat and extract .history
    ...
    /**
     * FIX: Upload gambar jadwal kelas via Multipart (untuk JadwalPembelajaranGuru).
     * Endpoint: POST /classes/{class}/schedule-image
     * @param classId   ID kelas dari tabel classes
     * @param imagePart MultipartBody.Part berisi file gambar (png/jpg)
     */
     ...
     * Mengembalikan URL string gambar jadwal dari server.
     * Response JSON contoh: {"data":{"url":"http://..."}} atau {"url":"http://..."}
     */
    ...
    /**
     * Parse URL gambar dari berbagai kemungkinan format JSON response:
     * {"data":{"url":"..."}}
     * {"data":{"path":"..."}}
     * {"url":"..."}
     * {"path":"..."}
     * {"data":"http://..."}  (string langsung)
     */
    private fun parseImageUrl(json: String): String? {
        return try {
            val obj = org.json.JSONObject(json)
            // Coba ambil dari "data" dulu
            ...
            // Fallback: jika JSON adalah string URL langsung
