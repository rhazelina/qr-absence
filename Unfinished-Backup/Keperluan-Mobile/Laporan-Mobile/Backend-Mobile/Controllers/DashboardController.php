...
class DashboardController extends Controller
{
    ...
    public function adminSummary(Request $request): JsonResponse
    {
        ...
        $stats = Cache::remember(DashboardCache::key('admin', null, $today), 600, function () use ($today) {
            ...
            return [
                ...
                'attendance_today' => [
                    'hadir' => $this->presentCountFromSummary($todayStats),
    ...
    public function studentDashboard(Request $request): JsonResponse
    {
        ...
        $data = Cache::remember(DashboardCache::key('student', $user->id, $today), 60, function () use ($user, $today) {
            ...
            $dayName = ScheduleDay::indonesian();
            $todayDayVariants = ScheduleDay::variants();

            // Get today's schedules for student's class
            $schedules = ScheduleItem::whereHas('dailySchedule', function ($q) use ($student, $todayDayVariants) {
    ...
    public function teacherDashboard(Request $request): JsonResponse
    {
        ...
        $data = Cache::remember(DashboardCache::key('teacher', $user->id, $today), 60, function () use ($user, $today) {
            ...
            $scheduleToday = $schedules->map(function ($schedule) use ($attendances, $studentsOnLeave, $studentCounts) {
                ...
                // Count by status
                $stats = [
                    'present' => $scheduleAttendances->whereIn('status', $this->presentLikeStatuses())->count(),
            ...
            $attendanceSummary = [
                'present' => $attendances->whereIn('status', $this->presentLikeStatuses())->count(),
            ...
            // School-wide attendance summary for today (same basis as admin dashboard)
            $schoolTodaySummary = Attendance::whereDate('date', $today)
                ->where('attendee_type', 'student')
                ->selectRaw('status, count(*) as count')
                ->groupBy('status')
                ->get()
            ...
            $schoolAttendanceSummaryToday = [
                'present' => (int) $schoolTodaySummary->get(AttendanceStatus::PRESENT->value, 0),
                'excused' => (int) $schoolTodaySummary->get(AttendanceStatus::EXCUSED->value, 0)
                    + (int) $schoolTodaySummary->get(AttendanceStatus::PERMISSION->value, 0),
                'sick' => (int) $schoolTodaySummary->get(AttendanceStatus::SICK->value, 0),
                'absent' => (int) $schoolTodaySummary->get(AttendanceStatus::ABSENT->value, 0),
            ];
            ...
            $payload = [
                ...
                'today_statistics' => [
                    'present' => $attendanceSummary['present'] + $attendanceSummary['late'],
                    'absent' => $attendanceSummary['absent'],
                    'late' => $attendanceSummary['late'],
                    'excused' => $attendanceSummary['izin'],
                    'sick' => $attendanceSummary['sick'],
                    'total_classes' => $scheduleToday->count(),
                ],
                'school_attendance_summary_today' => $schoolAttendanceSummaryToday,
                'schedule_today' => $scheduleToday,
            ];
    ...
    public function homeroomDashboard(Request $request): JsonResponse
    {
        ...
        $todayDayVariants = ScheduleDay::variants($today);

        // Get today's schedules for homeroom class
        $schedules = ScheduleItem::whereHas('dailySchedule', function ($q) use ($homeroomClass, $todayDayVariants) {
            $q->whereIn('day', $todayDayVariants)
        ...
        $present = $this->presentCountFromSummary($attendanceSummary);
        $late = (int) $attendanceSummary->get(AttendanceStatus::LATE->value, 0);
        $sick = (int) $attendanceSummary->get(AttendanceStatus::SICK->value, 0);
        $excused = (int) $attendanceSummary->get(AttendanceStatus::EXCUSED->value, 0)
            + (int) $attendanceSummary->get(AttendanceStatus::PERMISSION->value, 0);
        $absent = (int) $attendanceSummary->get(AttendanceStatus::ABSENT->value, 0);
        ...
        $todaySummary = [
            ...
            'excused' => $excused,
            'absent' => $absent,
            'attendance_rate' => $totalStudents > 0 ? (($present + $late) / $totalStudents) * 100 : 0,
        ];
        ...
        $payload = [
            ...
            'class' => [
                'id' => $homeroomClass->id,
                'name' => $homeroomClass->name,
                'grade' => $homeroomClass->grade ?? null,
                'major' => $homeroomClass->major?->name ?? null,
            ],
            'today_summary' => $todaySummary,
            'weekly_summary' => null,
            'students_absent_today' => null,
        ];
    ...
    public function wakaDashboard(Request $request): JsonResponse
    {
        ...
        $data = Cache::remember($cacheKey, 600, function () use ($today, $semesterId) {
            ...
            while ($currentMonth->lte($rangeEnd)) {
                ...
                $present = $monthRecords->whereIn('status', $this->presentLikeStatuses())->sum('count');
            ...
            while ($currentDay->lte($endOfWeek)) {
                ...
                $dailyData[] = [
                    ...
                    'hadir' => $dayRecords->whereIn('status', $this->presentLikeStatuses())->sum('count'),
    ...
    public function classDashboard(Request $request): JsonResponse
    {
        ...
        $data = Cache::remember(DashboardCache::classKey($classId, $today), 600, function () use ($today, $classId) {
            ...
            $monthlyData = Attendance::whereBetween('date', [$sixMonthsAgo, $today])
                ...
                ->selectRaw("
                    {$monthSelect} as month_key,
                    SUM(CASE WHEN status IN ('".AttendanceStatus::PRESENT->value."', '".AttendanceStatus::LATE->value."', 'dispensasi', 'dispen') THEN 1 ELSE 0 END) as present,
    ...
    {
        if (in_array(strtolower((string) $status), ['dispensasi', 'dispen'], true)) {
            $status = AttendanceStatus::PRESENT->value;
        }

        return AttendanceStatus::labelFor($status);
    }

    private function presentLikeStatuses(): array
    {
        return [AttendanceStatus::PRESENT->value, AttendanceStatus::LATE->value, 'dispensasi', 'dispen'];
    }

    private function presentCountFromSummary(\Illuminate\Support\Collection $summary): int
    {
        return (int) $summary->get(AttendanceStatus::PRESENT->value, 0)
            + (int) $summary->get('dispensasi', 0)
            + (int) $summary->get('dispen', 0);
    }
}
