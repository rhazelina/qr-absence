...
class TeacherController extends Controller
{
    ...
    public function myHomeroomSchedules(Request $request): JsonResponse
    {
        ...
        if ($request->filled('date')) {
            $dayVariants = ScheduleDay::variants((string) $request->date);
    ...
    public function myHomeroomAttendance(Request $request): JsonResponse
    {
        try {
            ...
            $query = \App\Models\Attendance::where('attendee_type', 'student')
                ->whereHas('student', function ($q) use ($classId) {
                    $q->where('class_id', $classId);
                });

            // Terima start_date/end_date (format Android) ATAU from/to (format lama)
            ...
            $items = $query
                ->with(['student.user', 'schedule.dailySchedule.classSchedule.class', 'attachments'])
                ->latest('date')
                ->get();

            // Gunakan AttendanceResource agar format JSON konsisten dengan endpoint lain
            return \App\Http\Resources\AttendanceResource::collection($items)->response();
        } catch (\Exception $e) {
    ...
    public function myHomeroomAttendanceSummary(Request $request): JsonResponse
    {
        ...
        $summary = $query->selectRaw('status, count(*) as count')
            ...
            ->get()
            ->pluck('count', 'status');
        $summary = $this->mergeDispensationIntoPresent($summary);

        $recapItems = (clone $query)
            ->with(['student.user:id,name', 'schedule:id,title,keterangan'])
            ->orderBy('date')
            ->get()
            ->map(function ($attendance) {
                return [
                    'date' => Carbon::parse($attendance->date)->format('d/m/Y'),
                    'student' => optional($attendance->student?->user)->name,
                    'status' => in_array(strtolower((string) $attendance->status), ['dispensasi', 'dispen'], true)
                        ? AttendanceStatus::PRESENT->value
                        : $attendance->status,
                    'keterangan' => $this->buildRemarks($attendance->reason, $attendance->schedule?->keterangan),
                ];
            });
    ...
    private function mergeDispensationIntoPresent(\Illuminate\Support\Collection $summary): \Illuminate\Support\Collection
    {
        $present = (int) $summary->get(AttendanceStatus::PRESENT->value, 0)
            + (int) $summary->get('dispensasi', 0)
            + (int) $summary->get('dispen', 0);

        $summary->forget(['dispensasi', 'dispen']);
        $summary->put(AttendanceStatus::PRESENT->value, $present);

        return $summary;
    }

    private function buildRemarks(?string $reason, ?string $scheduleRemark): ?string
    {
        $parts = array_filter([
            $reason ? trim($reason) : null,
            $scheduleRemark ? trim($scheduleRemark) : null,
        ]);
