...
class StudentDashboardController extends Controller
{
    ...
    public function scheduleToday(Request $request): JsonResponse
    {
        ...
        $today = now();
        $dayName = ScheduleDay::indonesian($today->toDateString());
        $dayVariants = ScheduleDay::variants($today->toDateString());
        $dateStr = $today->toDateString();

        $schedules = ScheduleItem::whereHas('dailySchedule', function ($query) use ($dayVariants, $profile) {
