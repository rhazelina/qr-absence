...
class TeacherScheduleDetailController extends Controller
{
    ...
    public function show(Request $request, \App\Models\ScheduleItem $schedule): JsonResponse
    {
        ...
        $manualSession = $this->attendanceService->getManualSessionState($schedule->id, $today);
        $eligibleStudentIds = $this->attendanceService
            ->eligibleStudentsForManualSession($schedule, $today)
            ->pluck('id')
            ->all();
        ...
        foreach ($students as $student) {
            ...
            $studentList[] = [
                ...
                'attendance' => $attendance ? [
                    ...
                    'is_draft' => (bool) $attendance->is_draft,
                    'draft_saved_at' => $attendance->draft_saved_at?->toIso8601String(),
                    'finalized_at' => $attendance->finalized_at?->toIso8601String(),
                    'manual_session_started_at' => $attendance->manual_session_started_at?->toIso8601String(),
                    'source' => $attendance->source,
                    'auto_late' => $attendance->source === 'manual_draft'
                        && $attendance->status === 'late'
                        && $attendance->manual_session_started_at !== null,
                ] : null,
                'last_status_today' => $this->formatLastStatus($latestAttendancesToday->get($student->id)),
            ];
        }

        $manualSession['date'] = $today;
        $manualSession['eligible_student_count'] = count($eligibleStudentIds);
        $manualSession['unfilled_student_count'] = count(array_diff($eligibleStudentIds, array_keys($attendances->all())));
        ...
        return response()->json([
            ...
            'date' => $today,
            'manual_attendance_session' => $manualSession,
            'statistics' => $stats,
