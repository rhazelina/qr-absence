  ...
  useEffect(() => {
    const fetchStudents = async () => {
      if (kelas && jadwalId && tanggal) {
        setLoading(true);
        try {
          const response = await apiService.request(`/me/schedules/${jadwalId}/detail`);
          if (response && response.students) {
            const mappedData = response.students.map((s, index) => {
              ...
              return {
                ...
                nisn: s.nisn || s.nis,
                nama: s.name,
                status: currentStatus || 'Hadir',
                keterangan: s.attendance && s.attendance.reason ? s.attendance.reason : '',
                dokumen: null
              };
            });
  ...
  const handleSimpan = async () => {
    ...
    const payload = {
      ...
      items: siswaList.map(s => {
        let beStatus = s.status.toLowerCase();
