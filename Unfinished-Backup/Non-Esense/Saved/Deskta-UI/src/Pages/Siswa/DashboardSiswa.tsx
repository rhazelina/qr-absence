...
import SiswaLayout from "../../component/Siswa/SiswaLayout.tsx";
...
import AbsensiSiswa from "./AbsensiSiswa.tsx";
...
// API-driven data — static dummy data removed
...
export default function DashboardSiswa({ user, onLogout }: DashboardSiswaProps) {
  ...
  // API state — replaces hardcoded name mapping and dummy stats
  const [displayName, setDisplayName] = useState(user.name);
  const [displayNISN, setDisplayNISN] = useState(user.phone || '');
  const [apiSchedules, setApiSchedules] = useState<ScheduleItem[] | null>(null);
  const [monthlyTrendData, setMonthlyTrendData] = useState([
    { month: "Jan", hadir: 0, izin: 0, sakit: 0, alpha: 0, pulang: 0 },
  ]);
  const [weeklyStats, setWeeklyStats] = useState({
    hadir: 0, izin: 0, sakit: 0, alpha: 0, pulang: 0,
  });

  const userWithName = { name: displayName, phone: user.phone };
  ...
  useEffect(() => {
    ...
    api.get('/me').then((data) => {
      ...
      const profile = (d.student_profile as Record<string, unknown>) || d;
      setDisplayName((d.name as string) || user.name);
      setDisplayNISN((profile.nisn as string) || (d.username as string) || user.phone || '');
    }).catch(() => { });

    /** Fetch today's schedule for siswa */
    api.get('/me/schedules/today').then((data) => {
      const rows = (Array.isArray(data) ? data : (data as Record<string, unknown>).data) as Record<string, unknown>[];
      if (Array.isArray(rows)) {
        setApiSchedules(rows.map((s, i) => ({
          id: String((s.id as string | number) ?? i),
          mapel: (s.subject_name as string) || (s.mapel as string) || 'Mata Pelajaran',
          guru: (s.teacher_name as string) || (s.guru as string) || '',
          start: (s.start_time as string) || (s.start as string) || '',
          end: (s.end_time as string) || (s.end as string) || '',
        })));
      }
    }).catch(() => { });

    /** Fetch dashboard summary for weekly stats */
    api.get('/me/dashboard/summary').then((data) => {
      const d = data as Record<string, unknown>;
      const attendance = (d.attendance as Record<string, unknown>) || d;
      setWeeklyStats({
        hadir: (attendance.present as number) || (attendance.hadir as number) || 0,
        izin: (attendance.permission as number) || (attendance.izin as number) || 0,
        sakit: (attendance.sick as number) || (attendance.sakit as number) || 0,
        alpha: (attendance.absent as number) || (attendance.alpha as number) || 0,
        pulang: (attendance.early_leave as number) || (attendance.pulang as number) || 0,
      });
    }).catch(() => { });

    /** Fetch monthly attendance trend */
    api.get('/me/attendance/summary').then((data) => {
      const rows = (Array.isArray(data) ? data : (data as Record<string, unknown>).data) as Record<string, unknown>[];
      if (Array.isArray(rows) && rows.length > 0) {
        setMonthlyTrendData(rows.map((r) => ({
          ...
          alpha: (r.absent as number) || (r.alpha as number) || 0,
                ...
                <div style={{
                  ...
                  <div>
                    ...
                    <div style={{
                      ...
                      {(apiSchedules ?? []).length} mata pelajaran dijadwalkan
                    </div>
                  </div>
                  <div style={{
                    display: "flex",
