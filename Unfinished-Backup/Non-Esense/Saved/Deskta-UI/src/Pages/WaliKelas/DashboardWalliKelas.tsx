...
// DUMMY_SCHEDULE removed — replaced by API
...
export default function DashboardWalliKelas({
  ...
  // API state — homeroom class info and real schedule
  const [homeroomName, setHomeroomName] = useState('Memuat...');
  ...
  useEffect(() => {
    ...
    api.get('/me/homeroom').then((data) => {
      const d = data as Record<string, unknown>;
      const className = (d.class_name as string) || (d.name as string) || (d.homeroom_class as string) || '';
      if (className) { setHomeroomName(className); }
      const students = (d.total_students as number) || (d.students_count as number) || null;
      if (students !== null) { setTotalStudents(students); }
    }).catch(() => { setHomeroomName('–'); });

    /** Fetch today's teaching schedule for walikelas */
    api.get('/me/schedules/today').then((data) => {
      ...
      if (Array.isArray(rows)) {
        setSchedules(rows.map((s, idx) => ({
          id: String((s.id as string | number) ?? idx),
          subject: (s.subject_name as string) || (s.mapel as string) || 'Mata Pelajaran',
          className: (s.class_name as string) || (s.kelas as string) || '',
          jurusan: (s.class_name as string) || '',
          jam: (s.time as string) || (s.jam as string) || '',
        })));
      }
    }).catch(() => { });
            ...
            <div style={styles.mainContainer(isMobile)}>
              ...
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {schedules.length === 0 ? (
                    <div style={{ color: '#6B7280', textAlign: 'center', padding: '12px' }}>Tidak ada jadwal hari ini</div>
                  ) : schedules.map((item) => (
                    <div
                      key={item.id}
