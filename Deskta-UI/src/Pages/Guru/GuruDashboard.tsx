...
// DUMMY_SCHEDULE removed — fetched from API
...
export default function DashboardGuru({ user, onLogout }: DashboardGuruProps) {
  ...
  useEffect(() => {
    ...
    api.get('/me/schedules/today')
      .then((data) => {
        const rawSchedules = (Array.isArray(data) ? data : (data as Record<string, unknown>).data) as Record<string, unknown>[];
        if (Array.isArray(rawSchedules)) {
          setSchedules(rawSchedules.map((s, idx) => ({
            id: String((s.id as number | string) ?? idx),
            subject: (s.subject_name as string) || (s.subject as string) || (s.mapel as string) || 'Mata Pelajaran',
            className: (s.class_name as string) || (s.kelas as string) || '',
            jurusan: (s.class_name as string) || (s.kelas as string) || '',
            ...
            <div style={styles.mainContainer(isMobile)}>
              ...
              <div style={styles.topGrid(isMobile)}>
                ...
                <div style={styles.totalMengajarCard(isMobile)}>
                  ...
                  <div style={styles.totalBadge(isMobile)}>
                    {isLoadingSchedule ? 'Memuat...' : `${schedules.length} Kelas`}
                  </div>
                </div>
              </div>
              ...
              <div style={styles.scheduleGrid(isMobile)}>
                {schedules.length === 0 && !isLoadingSchedule ? (
                  <div style={{ textAlign: 'center', color: '#6B7280', padding: '20px' }}>Tidak ada jadwal hari ini</div>
