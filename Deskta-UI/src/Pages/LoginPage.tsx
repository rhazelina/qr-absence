...
export default function LoginPage({ role, onLogin, onBack }: LoginPageProps) {
  ...
  // ==================== HANDLE FORM SUBMIT ====================
  /** Map backend role values to Deskta internal role keys */
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Siswa & pengurus_kelas tidak perlu password
    const needsPassword = role === 'admin' || role === 'guru' || role === 'waka' || role === 'wakel';
    ...
    try {
      ...
      const body: Record<string, string> = { login: form.identifier.trim() };
      ...
      const data = await response.json() as { token?: string; user?: Record<string, unknown>; message?: string; errors?: Record<string, string[]> };
      ...
      const desktaRole = ROLE_MAP[backendRole] || backendRole;

      // Persist token + user data
      localStorage.setItem('token', data.token);
      localStorage.setItem('userData', JSON.stringify(data.user));
      localStorage.setItem('userRole', desktaRole);

      const userName = (data.user.name as string) || form.identifier.trim();
      const userPhone = (data.user.phone as string) || (data.user.username as string) || form.identifier.trim();

      if (role) {
        onLogin(desktaRole, userName, userPhone);
      } else {
