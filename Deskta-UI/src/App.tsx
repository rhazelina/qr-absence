...
export default function App() {
  ...
  // Restore user from token-based localStorage (matches Website-UI keys)
  useEffect(() => {
    try {
      ...
      if (token && userDataRaw && role) {
        const userData = JSON.parse(userDataRaw) as Record<string, unknown>;
        setCurrentUser({
          role,
          name: (userData.name as string) || "",
          phone: (userData.phone as string) || (userData.username as string) || "",
        });
      } else {
        // Fallback: legacy currentUser key
        const legacy = localStorage.getItem("currentUser");
        if (legacy) {
          const parsed = JSON.parse(legacy) as { role: string; name: string; phone: string };
