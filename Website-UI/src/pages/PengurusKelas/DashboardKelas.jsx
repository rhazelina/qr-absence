  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 1. Fetch dashboard data (Stats & Trend)
        const dashData = await api.get('/me/class/dashboard');

        if (dashData) {
          if (dashData.profile) {
            setProfileData({
              name: dashData.profile.name || '',
              kelas: dashData.profile.kelas || '',
              id: dashData.profile.id || '',
              gender: dashData.profile.gender || 'perempuan'
            });

            const classId = dashData.profile.class_id;
            if (classId) {
              api.get(`/classes/${classId}/schedule-image`)
                .then(imgData => {
          ...
          const daily = dashData.dailyStats || dashData.statistik || {};
          setDailyStats({
            hadir: daily.hadir || 0,
            izin: daily.izin || 0,
            sakit: daily.sakit || 0,
            alpha: daily.alpha || 0,
            terlambat: daily.terlambat || 0,
            pulang: daily.pulang || 0
          });

          const trend = dashData.monthlyTrend || dashData.trend || [];
          setMonthlyTrend(Array.isArray(trend) ? trend : []);
        }

        // 2. Fetch today's schedule to get total subjects
        try {
          const schedules = await api.get('/me/class/schedules');
          const todayName = new Date().toLocaleDateString('en-US', { weekday: 'long' });
          const todaySchedules = (Array.isArray(schedules) ? schedules : []).filter(s => s.day === todayName || s.day_name === todayName);
          setTotalSubjects(todaySchedules.length);
        } catch (err) {
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', flex: '1' }}>
                      {[
                        ...
                        <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          ...
                          <span style={{ fontSize: '11px', color: '#6b7280', fontWeight: '600' }}>{item.label}</span>
                          <span style={{ fontSize: '13px', fontWeight: 'bold', color: '#1f2937', marginLeft: 'auto' }}>{item.value}</span>
                        </div>
                      ))}
                    </div>
                  </div>
