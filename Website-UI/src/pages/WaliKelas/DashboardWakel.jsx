...
const DashboardWakel = () => {
  ...
  useEffect(() => {
    const updateDateTime = () => {
      ...
      setCurrentFormattedDate(`${date}-${month}-${year} (${dayName})`);
    };
  ...
  useEffect(() => {
    const loadCompletedAbsensi = async () => {
      if (scheduleData.length === 0) return;

      const completed = new Set();

      await Promise.all(scheduleData.map(async (jadwal) => {
        try {
          const result = await api.get(`/me/schedules/${jadwal.id}/detail`);
          if (result && result.statistics) {
            const { present, late, sick, izin, absent, return: pulang, on_leave } = result.statistics;
            const totalAttended = (present || 0) + (late || 0) + (sick || 0) + (izin || 0) + (absent || 0) + (pulang || 0) + (on_leave || 0);
            if (totalAttended > 0) {
              completed.add(jadwal.id);
            }
          }
        } catch (error) {
          console.error(`Gagal mendapatkan info detail untuk jadwal ${jadwal.id}`);
        }
      }));
    ...
    if (currentFormattedDate && scheduleData.length > 0) {
      loadCompletedAbsensi();
    }

    const handleVisibilityChange = () => {
      if (!document.hidden && currentFormattedDate && scheduleData.length > 0) {
        loadCompletedAbsensi();
      }
    };
  ...
  }, [currentFormattedDate, scheduleData]);
  ...
  const fetchData = async () => {
    try {
      // Fetch profil wali kelas (dari /me)
      const userRes = await api.get('/me');
      setWaliKelas({
        nama: userRes.name || '',
        nip: userRes.profile?.nip || userRes.username || '',
        role: 'Wali Kelas'
      });

      // Fetch jadwal hari ini
      const schedulesRes = await api.get('/me/schedules/today');
      const schedules = schedulesRes.data || schedulesRes || [];
      const mapped = Array.isArray(schedules) ? schedules.map(s => ({
        id: s.id,
        mataPelajaran: s.subject?.name || s.subject_name || s.name || '',
        kelas: s.class?.name || s.class_name || '',
        jamKe: s.period || s.time_slot || '',
        waktu: s.start_time ? `${s.start_time} - ${s.end_time}` : s.time || '',
      })) : [];
      setScheduleData(mapped);
      setStats(prev => ({ ...prev, totalKelas: mapped.length }));

      // Fetch info homeroom & jumlah siswa
      const studentsRes = await api.get('/me/homeroom/students');
