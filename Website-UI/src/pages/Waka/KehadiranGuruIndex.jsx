...
function KehadiranGuruIndex() {
  ...
  const statusConfig = {
    ...
    present: { bg: 'status-hadir', icon: 'fa-check-circle' },
    Terlambat: { bg: 'status-terlambat', icon: 'fa-clock' },
    late: { bg: 'status-terlambat', icon: 'fa-clock' },
    Izin: { bg: 'status-izin', icon: 'fa-info-circle' },
    excused: { bg: 'status-izin', icon: 'fa-info-circle' },
    Sakit: { bg: 'status-sakit', icon: 'fa-heartbeat' },
    sick: { bg: 'status-sakit', icon: 'fa-heartbeat' },
    Alfa: { bg: 'status-alfa', icon: 'fa-times-circle' },
    alpha: { bg: 'status-alfa', icon: 'fa-times-circle' },
    Pulang: { bg: 'status-pulang', icon: 'fas fa-sign-out-alt' },
    early_leave: { bg: 'status-pulang', icon: 'fas fa-sign-out-alt' },
  ...
  const fetchKehadiranGuru = async () => {
    try {
      setLoading(true);
      const result = await api.get(`/waka/attendance/teachers/daily?date=${filterTanggal}`);

      const raw = result.items?.data || result.items || [];
      const mapped = (Array.isArray(raw) ? raw : []).map(k => ({
        id: k.teacher?.id || k.id,
        guru: {
          id: k.teacher?.id,
          kode_guru: k.teacher?.nip || k.teacher?.user_id || '',
          nama: k.teacher?.user?.name || k.teacher?.name || '',
          kelas: k.attendances?.[0]?.schedule?.daily_schedule?.class_schedule?.class?.name || '-'
        },
        jam: k.slots || Array(10).fill(null)
      }));
  ...
  const handleDelete = async (id) => {
    try {
      await api.delete(`/waka/attendance/teachers/${id}`);
