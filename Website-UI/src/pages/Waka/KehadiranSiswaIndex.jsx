  ...
  useEffect(() => {
    ...
    fetch(`${baseURL}/classes`, { headers })
      ...
      .then(result => {
        ...
        // Map backend fields to what the table expects
        const mapped = (Array.isArray(classes) ? classes : []).map(c => ({
          id: c.id,
          nama_kelas: c.name || c.nama_kelas || '',
          jurusan: { nama_jurusan: c.major?.code || c.major_name || c.jurusan?.nama_jurusan || '' },
          wali_kelas: c.homeroom_teacher?.name || c.wali_kelas || '-'
        }));
        setKelasList(mapped);
      })
