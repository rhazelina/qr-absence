...
export default function DashboardWaka() {
  ...
  useEffect(() => {
    ...
    // Fetch dashboard summary (total hadir, izin, sakit, alfa, pulang)
    fetch(`${baseURL}/waka/dashboard/summary`, { headers })
      .then(r => r.ok ? r.json() : null)
      .then(data => {
        if (!data) return;
        // Backend returns: { statistik: {hadir,izin,sakit,alpha,terlambat,pulang}, trend: [...], daily_stats: [...] }
        const s = data.statistik || {};
        // Apply today's overall stats to both semesters as fallback (no semester split in API)
        const stats = {
          hadir: s.hadir || 0,
          izin: s.izin || 0,
          sakit: s.sakit || 0,
          alfa: s.alpha || s.alfa || 0,
          pulang: s.pulang || 0
        };
        setStatistikData({ ganjil: stats, genap: stats });

        // Build chart from trend data (last 6 months)
        if (data.trend && Array.isArray(data.trend)) {
          const trend = data.trend;
          const labels = trend.map(t => t.month);
          const chartDatasets = [
            { label: 'Hadir', data: trend.map(t => t.present || t.hadir || 0), backgroundColor: '#22c55e' },
            { label: 'Izin', data: trend.map(t => t.izin || 0), backgroundColor: '#3b82f6' },
            { label: 'Sakit', data: trend.map(t => t.sick || t.sakit || 0), backgroundColor: '#f59e0b' },
            { label: 'Alfa', data: trend.map(t => t.absent || t.alpha || 0), backgroundColor: '#ef4444' },
          ];
          const builtChart = { labels, datasets: chartDatasets };
          setChartData({ ganjil: builtChart, genap: builtChart });
        }
      })
      .catch(err => console.error('Error fetching waka summary:', err));
  }, []);
