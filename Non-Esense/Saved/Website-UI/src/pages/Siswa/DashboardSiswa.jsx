...
// ==================== UTILITY FUNCTIONS ====================
const getTodaySubjectCount = (scheduleItems) => {
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const classId = profile.profile?.class_id || profile.student_profile?.class_id;
        setProfileData({
          name: profile.name || '',
          kelas: profile.profile?.class_name || profile.student_profile?.class?.name || '',
          id: profile.username || profile.id || '',
          gender: profile.gender || 'perempuan',
          studentId: profile.student_profile?.id
        });
        setProfileImage(profile.profile?.photo_url || null);

        // 2. Fetch today's schedule
        const scheduleRes = await api.get('/me/dashboard/schedule-today');
        const scheduleItems = scheduleRes.items || [];
        setScheduleItems(scheduleItems); // We need a new state for this or just use totalSubjects

        // 3. Fetch attendance stats (Weekly & Monthly Trend)
        const statsRes = await api.get('/me/dashboard/attendance-stats');
        if (statsRes) {
          // Weekly Stats
          if (statsRes.weekly_stats) {
            const ws = statsRes.weekly_stats;
            setWeeklyStats({
              hadir: ws.present || 0,
              terlambat: ws.late || 0,
              pulang: ws.pulang || ws.early_leave || 0,
              izin: ws.excused || ws.permission || 0,
              sakit: ws.sick || 0,
              alpha: ws.absent || 0
            });
          }
          ...
          // Monthly Trend
          if (statsRes.monthly_chart) {
            const trend = statsRes.monthly_chart.map(item => ({
              month: item.day_label || item.date,
              percentage: item.total > 0 ? Math.round((item.present / item.total) * 100) : (item.present > 0 ? 100 : 0),
              hadir: item.present,
              total: item.total || (item.present + item.absent + item.excused + item.sick)
            }));
            setMonthlyTrend(trend);
          }
        }

        // 4. Fetch schedule image if classId exists
        if (classId) {
          const imgRes = await api.get(`/classes/${classId}/schedule-image`).catch(() => null);
          if (imgRes?.url) {
            setScheduleData({ scheduleImageUrl: imgRes.url });
          }
        }
  ...
  const totalSubjects = getTodaySubjectCount(scheduleItems);
