...
function KehadiranSiswaShow() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await api.get(`/waka/classes/${id}/attendance?date=${today}`);

      setKelas({
        nama_kelas: res.class?.name || '',
        wali_kelas: res.class?.homeroom_teacher?.user?.name || '-'
      });

      // Flatten data for table: Each schedule item has attendances
      const flattened = [];
      if (Array.isArray(res.items)) {
        res.items.forEach(item => {
          const subjectName = item.schedule?.subject?.name || '';
          const jamPelajaran = `${item.schedule?.start_time || ''} - ${item.schedule?.end_time || ''}`;

          if (Array.isArray(item.attendances)) {
            item.attendances.forEach(att => {
              flattened.push({
                id: att.id, // attendance record primary key
                student_id: att.student_id,
                schedule_id: att.schedule_id,
                nisn: att.student?.nis || att.student?.nisn || '',
                nama: att.student?.user?.name || att.student?.nama || '',
                mata_pelajaran: subjectName,
                status: mapBackendStatus(att.status),
                keterangan: att.reason || '',
                jam_pelajaran: jamPelajaran,
                tanggal: res.date,
                bukti_foto: att.document_url || null
              });
            });
          }
        });
      ...
      setSiswaList(flattened);
      setStatusCounts(hitungStatusCounts(flattened));
    } catch (err) {
      console.error('Error fetching class daily attendance:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStatusUpdate = async () => {
    try {
      ...
      await api.post('/attendance/manual', {
        ...
        schedule_id: selectedSiswa.schedule_id,
        date: selectedSiswa.tanggal,
        status: mapFrontendStatusToBe(selectedStatus)
      });
