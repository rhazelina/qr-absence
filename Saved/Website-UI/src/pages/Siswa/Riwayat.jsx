    ...
    const fetchProfile = async () => {
      try {
        ...
        setCurrentStudent({
          ...
          name: profile.name,
          nis: profile.profile?.nis || profile.username || ''
        });
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const fetchAttendanceData = async () => {
      try {
        ...
        const result = await api.get(`/me/attendance?from=${startDate}&to=${endDate}`);
        const records = result.data || result || [];

        const formattedRecords = (Array.isArray(records) ? records : []).map(record => {
          const statusStr = mapBackendStatus(record.status || '');
          return {
            recordDate: record.date,
            date: formatDisplayDate(record.date),
            period: record.schedule?.time_slot?.name || record.schedule?.period || '',
            subject: record.schedule?.subject?.name || record.subject_name || '',
            teacher: record.schedule?.teacher?.user?.name || record.teacher_name || '',
            status: statusStr,
            statusColor: getStatusColor(statusStr),
            reason: record.reason || record.notes || null,
            proofImage: record.attachments?.[0]?.file_url || record.proof_image_url || null,
          };
        });
        ...
        // 2. Fetch stats
        try {
          const statsRes = await api.get(`/me/attendance/summary?from=${startDate}&to=${endDate}`);
          if (statsRes && statsRes.data && statsRes.data.statistik) {
            const s = statsRes.data.statistik;
            setStats({
              hadir: s.hadir || 0,
