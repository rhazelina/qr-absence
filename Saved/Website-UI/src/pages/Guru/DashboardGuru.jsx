...
function DashboardGuru() {
  ...
  const [profileData, setProfileData] = useState({ name: '', kode_guru: '' });
  ...
  useEffect(() => {
    const loadCompletedAbsensi = async () => {
      ...
      await Promise.all(allJadwal.map(async (jadwal) => {
        try {
          const response = await apiService.request(`/me/schedules/${jadwal.id}/detail`);
          if (response && response.statistics) {
            const { present, late, sick, izin, absent, return: pulang, on_leave } = response.statistics;
    ...
    if (currentFormattedDate && allJadwal.length > 0) {
      loadCompletedAbsensi();
    }

    // Event listener untuk refresh ketika kembali dari halaman presensi
    const handleVisibilityChange = () => {
      if (!document.hidden && currentFormattedDate && allJadwal.length > 0) {
        loadCompletedAbsensi();
      }
    };
  ...
  }, [currentFormattedDate, allJadwal]);
  ...
  // Fetch profile dan jadwal hari ini dari backend
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setProfileData({
          name: profile.name || '',
          kode_guru: profile.profile?.nip || profile.username || ''
        });
        ...
        // Map ke format yang digunakan UI
