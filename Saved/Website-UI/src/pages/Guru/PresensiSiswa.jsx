...
function PresensiSiswa() {
  ...
  // ✅ Initialize siswa list - data akan dimuat dari API
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      if (kelas && jadwalId && tanggal) {
        setLoading(true);
        try {
          const response = await api.get(`/me/schedules/${jadwalId}/detail`);
          if (response && response.students) {
            const mappedData = response.students.map((s, index) => {
              // Normalisasi status dari backend ke UI
              let currentStatus = '';
              // Backend returns 'present', 'late', 'sick', 'excused'/'izin', 'absent', 'return'
              if (s.attendance) {
                const stat = s.attendance.status.toLowerCase();
                if (stat === 'present') currentStatus = 'hadir';
                else if (stat === 'late') currentStatus = 'terlambat';
                else if (stat === 'sick') currentStatus = 'sakit';
                else if (stat === 'excused' || stat === 'izin') currentStatus = 'izin';
                else if (stat === 'absent' || stat === 'alpha') currentStatus = 'alfa';
                else if (stat === 'return') currentStatus = 'pulang';
                else currentStatus = stat;
              }
              ...
              return {
                ...
                keterangan: s.attendance && s.attendance.reason ? { text: s.attendance.reason } : null,
                dokumen: null // Attachment visualization not fully supported in bulk load yet
              };
            });
          ...
          alert("Gagal memuat daftar siswa. Silakan coba lagi.");
        } finally {
          setLoading(false);
        }
      }
  ...
  const handleSimpan = async () => {
    ...
    // Format proper tanggal to YYYY-MM-DD for backend
    // Assuming 'tanggal' might be formatted like DD/MM/YYYY or something, we should just use the current date if 'tanggal' is strictly for UI
    ...
    const payload = {
      ...
      items: siswaList.map(s => {
        // Map status to what backend expects
        let beStatus = s.status;
        if (beStatus === 'hadir') beStatus = 'present';
        if (beStatus === 'alfa') beStatus = 'absent';
        if (beStatus === 'terlambat') beStatus = 'late';
        if (beStatus === 'pulang') beStatus = 'return';
        if (beStatus === 'sakit') beStatus = 'sick';
        if (beStatus === 'izin') beStatus = 'excused';

        return {
          student_id: s.id,
          status: beStatus,
          reason: s.keterangan ? (s.keterangan.alasan || s.keterangan.text) : null
        };
      })
    };
    ...
    try {
      setLoading(true);
      await apiService.post('/attendance/bulk-manual', payload);
                ...
                <tr>
                  <td colSpan="9" style={{ textAlign: 'center', padding: '40px', color: '#666' }}>
                    <div>
                      ...
                      <p style={{ fontSize: '14px', marginTop: '5px' }}>Pastikan data kelas dan siswa sudah tersedia</p>
                    </div>
                  </td>
                </tr>
              ) : (
