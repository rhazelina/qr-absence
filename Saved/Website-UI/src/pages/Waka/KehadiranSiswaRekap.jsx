...
export default function KehadiranSiswaRekap() {
  ...
  const mapSummaryToFrontend = (backendData) => {
    if (!Array.isArray(backendData)) return [];
    return backendData.map(item => ({
      id: item.student.id,
      nisn: item.student.nis || item.student.nisn || '',
      nama: item.student.user?.name || item.student.nama || '',
      hadir: item.totals.present || 0,
      terlambat: item.totals.late || 0,
      izin: item.totals.excused || 0,
      sakit: item.totals.sick || 0,
      alfa: item.totals.alpha || 0,
      pulang: item.totals.early_leave || 0,
    }));
  };
  ...
  const handleSave = () => {
    ...
    alert('Simpan rekap berhasil (lokal). Untuk perubahan data permanen, silakan edit melalui menu Presensi Siswa.');
  };
  ...
  const fetchData = async (start, end) => {
    try {
      ...
      setKelasInfo({
        nama_kelas: classRes.data?.name || classRes.name || '',
      });

      // Fetch attendance summary
      const summaryRes = await api.get(`/waka/classes/${id}/attendance-summary?from=${start}&to=${end}`);

      // Backend wakaClassSummary handles pagination or collection. Adjusting if it's paginated.
      const rawData = summaryRes.data || summaryRes;
      const studentsData = Array.isArray(rawData) ? rawData : (rawData.data || []);

      setData(mapSummaryToFrontend(studentsData));
    } catch (err) {
  ...
  const handleApplyPeriode = () => {
    if (tanggalMulai && tanggalSampai) {
      fetchData(tanggalMulai, tanggalSampai);
    }
  };
