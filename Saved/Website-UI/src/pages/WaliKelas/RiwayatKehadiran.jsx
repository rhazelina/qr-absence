...
const RiwayatKehadiran = () => {
  ...
  const fetchClassInfo = async () => {
    try {
      const res = await api.get('/me/homeroom');
      setClassInfo(res);
      setClassName(res.name || '');
    } catch (err) {
      console.error('Error fetching homeroom class info:', err);
    }
  };

  // Fetch homeroom attendance when dates change or class info is loaded
  useEffect(() => {
    if (classInfo?.id) {
      fetchAttendanceData();
    }
  }, [startDate, endDate, classInfo]);

  const fetchAttendanceData = async () => {
    try {
      setIsLoading(true);
      const res = await api.get(`/classes/${classInfo.id}/students/attendance-summary?from=${startDate}&to=${endDate}`);
      ...
      const mapped = raw.map((item, idx) => ({
        ...
        izin: item.totals?.excused || item.totals?.permission || 0,
        alfa: item.totals?.alpha || item.totals?.absent || 0,
        pulang: item.totals?.early_leave || 0,
        details: item.details || [] // Assuming backend might provide details or we'll fetch them on click
      }));
