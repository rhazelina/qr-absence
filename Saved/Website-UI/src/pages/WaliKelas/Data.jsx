...
const Data = () => {
  ...
  const [kelasInfo, setKelasInfo] = useState({ nama: '' });
  ...
  // Fetch homeroom students and class info from API
  useEffect(() => {
    fetchData();
  }, []);
  ...
  const fetchData = async () => {
    try {
      ...
      const homeroomRes = await api.get('/me/homeroom');
      setKelasInfo({ nama: homeroomRes.name || '' });
      ...
      if (Array.isArray(students)) {
        const mapped = students.map(s => ({
          ...
          status: 'Hadir', // Default view in this page is often current state or reset
          keterangan: '',
          suratFile: null,
          suratFileName: null,
          wasTerlambat: false,
      ...
      // Fetch schedules for mapel list
      ...
      if (Array.isArray(schedules)) {
        const mapels = [...new Set(schedules.map(s => s.subject?.name || s.subject_name || s.name || '').filter(Boolean))];
