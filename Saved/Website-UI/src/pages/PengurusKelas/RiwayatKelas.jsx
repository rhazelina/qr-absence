...
function Riwayat() {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        const result = await api.get('/me/class/students');
        const students = result.data || result || [];
        setStudentList(Array.isArray(students) ? students.map(s => ({
          id: s.id,
          name: s.name || s.nama || s.user?.name,
          nis: s.nis || s.nisn || s.user?.id
        })) : []);
      } catch (error) {
    ...
    const fetchAttendanceData = async () => {
      try {
        ...
        const params = {
          from: startDate,
          to: endDate,
          ...(selectedStudent !== 0 && { student_id: selectedStudent })
        };
        const result = await api.get('/me/class/attendance', params);
        const records = result.data || result || [];

        // Format records
        const formattedRecords = (Array.isArray(records) ? records : []).map(record => ({
          recordDate: record.date,
          date: formatDateDisplay(record.date),
          period: record.time_slot || record.period || (record.schedule?.start_time ? `${record.schedule.start_time.slice(0, 5)} - ${record.schedule.end_time.slice(0, 5)}` : ''),
          subject: record.subject_name || record.subject || record.schedule?.subject?.name || '',
          teacher: record.teacher_name || record.teacher || record.schedule?.teacher?.user?.name || '',
          status: record.status_label || record.status || '',
          statusColor: getStatusColor(record.status_label || record.status || ''),
          reason: record.notes || record.reason || record.description || null,
          proofDocument: record.proof_document || record.proofDocument || null,
          proofImage: record.proof_image_url || record.proofImageUrl || (record.attachments && record.attachments[0]?.url) || null,
          studentId: record.student_id || record.studentId || record.student?.id,
          studentName: record.student_name || record.studentName || record.student?.user?.name || '',
          nis: record.nis || record.student?.nis || ''
        }));

        setAttendanceRecords(formattedRecords);

        // Hitung stats dari records (karena endpoint stats mungkin tidak dipisahkan)
        const counted = { hadir: 0, terlambat: 0, izin: 0, sakit: 0, alpha: 0, pulang: 0 };
        formattedRecords.forEach(r => {
          const s = (r.status || '').toLowerCase();
          if (s.includes('hadir') && !s.includes('lambat')) counted.hadir++;
          else if (s.includes('lambat') || s.includes('terlambat') || s.includes('late')) counted.terlambat++;
          else if (s.includes('izin') || s.includes('excused')) counted.izin++;
          else if (s.includes('sakit') || s.includes('sick')) counted.sakit++;
          else if (s.includes('alpha') || s.includes('alfa') || s.includes('absent')) counted.alpha++;
          else if (s.includes('pulang') || s.includes('return')) counted.pulang++;
        });
        setStats(counted);
