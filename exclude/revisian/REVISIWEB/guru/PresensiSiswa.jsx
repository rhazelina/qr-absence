...
function PresensiSiswa() {
  ...
  const getSiswaByKelas = (kelasNama) => {
    // TODO: Ganti dengan API call untuk mengambil data siswa berdasarkan kelas
    // Contoh: return fetch(`/api/siswa?kelas=${kelasNama}`).then(res => res.json());
    
    // Sementara return array kosong, nanti akan diisi dari backend
    return [];
  };
...
const handleStatusChange = (index, newStatus) => {
  ...
  const handleSimpan = () => {
    ...
    // Contoh: fetch('/api/presensi', { method: 'POST', body: JSON.stringify(siswaList) })
