...
class JadwalPembelajaranGuru : BaseNetworkActivity() {
    ...
    private fun loadSchedulesFromApi() {
        lifecycleScope.launch {
            try {
                val result = scheduleRepository.getMySchedules()
                handleResult(result,
                    onSuccess = { schedules ->
                        jadwalList.clear()
                        jadwalList.addAll(schedules.mapIndexed { index, schedule ->
                            JadwalItem(
                                (index + 1),
                                "${schedule.`class`?.name ?: "Unknown"} - ${schedule.subjectName ?: "Unknown"}",
                                null,
                                "jadwal_${index}.pdf"
                            )
                        })
                    ...
                    onError = { _, msg ->
                        showError("Gagal memuat jadwal: $msg")
                    }
                )
            } catch (e: Exception) {
