...
class DetailJadwalWakelActivity : BaseNetworkActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val jadwalData = intent.getSerializableExtra("JADWAL_DATA", DashboardGuruActivity.JadwalData::class.java)
    ...
    private fun loadStudentsFromApi() {
        ...
        lifecycleScope.launch {
            try {
                val result = if (scheduleId > 0) {
                    teacherRepository.getMyScheduleStudents(scheduleId)
                } else if (classId > 0) {
                    studentRepository.getStudents(classId = classId)
                } else {
                ...
                handleResult(result,
                    onSuccess = { students ->
                        ...
                        val txtJumlahSiswa: TextView = findViewById(R.id.txt_end_3)
                        txtJumlahSiswa.text = students.size.toString()
                    },
                    onError = { _, msg ->
                        showError(msg ?: "Gagal memuat data siswa")
                        loadedStudentNames = emptyArray()
                    }
                )
            } catch (e: Exception) {
