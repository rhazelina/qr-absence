...
class StatistikKehadiran : BaseNetworkActivity() {
    ...
    private fun loadAttendanceData() {
        ...
        // Calculate start date based on selectedTime
        when (selectedTime) {
            "Harian" -> { /* same day */ }
        ...
        android.util.Log.d("StatistikKehadiran", "Loading attendance: $startDate to $endDate, Role: $selectedRole")
        ...
        lifecycleScope.launch {
            try {
                ...
                android.util.Log.d("StatistikKehadiran", "API result: $result")
                handleResult(result,
                    onSuccess = { summary ->
                        android.util.Log.d("StatistikKehadiran", "Summary loaded: present=${summary.present}, late=${summary.late}, excused=${summary.excused}, sick=${summary.sick}, absent=${summary.absent}")
                        ...
                        pulang = hadir // Use hadir as pulang (API may not have separate pulang)
                        android.util.Log.d("StatistikKehadiran", "Updated values: hadir=$hadir, terlambat=$terlambat, izin=$izin, sakit=$sakit, alpha=$alpha")
                        updateBarChart()
                        updatePieChart()
                    },
                    onError = { _, err ->
                        android.util.Log.e("StatistikKehadiran", "API Error: $err")
                ...
                android.util.Log.e("StatistikKehadiran", "Exception: ${e.stackTraceToString()}")
                hadir = 0f; terlambat = 0f; izin = 0f; sakit = 0f; pulang = 0f; alpha = 0f
                updateBarChart()
                updatePieChart()
            }
